...
def find_best_match(headings, broken_anchor):
    ...
    broken_anchor = unquote(broken_anchor).lower().replace('-', ' ').replace('#', '')
    best_match = None
    max_similarity = 0
...
def main():
    ...
    for broken_link_info in broken_links:
        try:
            ...
            if broken_link_path == '/GuiaParaPadres/':
                target_file_path = 'index.md'
            else:
                target_file_path = broken_link_path.strip('/').split('/')[-1] + '.md'
